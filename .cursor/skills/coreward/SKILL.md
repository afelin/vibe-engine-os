...
- `VIBE_DEPTH` dials ContextPack radius (`maxHops` / `charBudget` / `allowLlm`) — wire env name.
...
**Honesty:** Coreward Mode is not a kernel IDE sandbox. Soft Cursor hooks remind (fail-open). Ward covers CI/promote when a Mandate is on. Visibility strip: `Ward LEGACY|ON · Mode OFF|ON · ticket fresh|expired`. Coreward = promotion gate + Agentic Cost Plane; CyberReady stays a sibling (optional sock). Gate candidates are stubs — **no compounding SLA** until human merge into `gates.json`. Cost claims: measure with `npm run coreward:cost-dogfood`.
